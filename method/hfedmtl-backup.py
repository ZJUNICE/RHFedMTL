...
def hfedmtl(data):
    ...
    hfedmtl_server_iters = int(config["HFedMTL"]["hfedmtl_server_iters"])
    hfedmtl_base_iters = int(config["HFedMTL"]["hfedmtl_base_iters"])
    H = int(config["HFedMTL"]["hfedmtl_local_iters"])
    ...
    for h in range(hfedmtl_server_iters):
        for hh in range(hfedmtl_base_iters):
            ...
            for b in range(task_num):
                ...
                for t in range(terminal_num):
                    for s in range(H):
                        ...
                        grad = lambda_sum * nb * (1.0 - update) / (
                            nb * curr_x * curr_x.T
                        ) + (alpha_old * curr_y)
                        grad = (
                            grad
                            - grad * lambda1 * (curr_x * local_w)
                            - 2 * lambda2 * (curr_x * r)
                        )
            ...
            # BS aggregate
            for b in range(task_num):
                W[:, [b]] += deltaB[:, [b]] / task_num + lambda2 * r / lambda_sum
