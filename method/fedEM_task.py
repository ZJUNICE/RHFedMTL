...
def fedEM(data):
    ...
    rmse = np.zeros((iterNum, task_num, 1))
    dual_objs = np.zeros((iterNum, task_num, 1))
    primal_objs = np.zeros((iterNum, task_num, 1))
    ...
    for h in range(sgd_server_iters):
        for hh in range(sgd_base_iters):
            ...
            rmse[iter] = compute_rmse_task(Xtest, Ytest, W)
            ...
            for b in range(task_num):
                ...
                for t in range(terminal_num):
                    for s in range(H):
                        ...
                        local_w = W[:, [idx%m]] + deltaW[idx%m][:, [t]]
                        # local_w = W[:, [b]]
                        ...
                        if update < 1:
                            ...
                            # grad = -lambda1 * local_w

                            grad = curr_y * curr_x.T
                            # print("grad: ", grad)
                            deltaW[idx%m][:, [t]] += grad / nb
                            # deltaB[:, [b]] += grad / (nb)
                            # W[:, [b]] += grad / nb
                            # print("Wb: ", W[:, [b]])
